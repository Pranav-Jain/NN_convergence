...
3. Same as the second experiment, but instead, an MLP is used to learn the exact normals, and the MLP is used to solve the Poisson equation.
4. Same as the third experiment, but instead, an MLP is used to learn the mesh normals, and the MLP is used to solve the Poisson equation.
...
## Run
Go to the required directory: __test_exact__, __test_exact_parameterization__, __test_computed_parameterization__.
...
```sh
python3 test_autodiff.py [1|2|....|15]
```
The last argument is the example number you want to run for.

### test_exact_parameterization
In the code, we choose 0.5*(x^2 + y^2) from [-1, 1] as the heightfield. If an ellipsoid is chosen, then the parameterization from the unit sphere to the ellipsoid is chosen to solve the Poisson equation.
...
After setting the _config.json_, run the following command:
```sh
python3 test_parameterization.py [1|2|....|15]
```
The last argument is the example number you want to run for.
